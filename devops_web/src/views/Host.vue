  ...
  <a-table :dataSource="hostList.data" :row-selection="rowSelection" row-key="id" :columns="hostFormColumns">
    <template #bodyCell="{ column, text, record }">
       <template v-if="column.dataIndex === 'console'">
        <router-link :to="`/devops/console/${record.id}`">console</router-link>
       </template>
      ...
      <a-form-item label="主机类别" name="category">
        <a-row>
          ...
          <a-col :span="8" :offset="2">
            <a-button @click="showHostCategoryFormModal" type="primary">
              新建类别
            </a-button>
          </a-col>
        </a-row>
    ...
    <a-modal v-model:visible="excelVisible" title="导入excel批量创建主机" @ok="onExcelSubmit" @cancel="excelFormCancel"
      ...
      <p>
        <a-form-item has-feedback label="模板下载" help="请下载使用该模板填充数据后导入">
          <a download="主机导入模板.xls" href="http://api.devops.cn:8000/static/HostTemplate.xlsx">主机导入模板.xls</a>
        </a-form-item>
      </p>
      <p>
...
export default {
  ...
  setup() {
    ...
    const onHostFormSubmit = () => {
      ...
      axios.post(`${settings.host}/host/hosts`, formData, {
            headers: {
    ...
    const deleteHost = record => {
      console.log(record);
      axios.delete(`${settings.host}/host/hosts/${record.id}`, {
        headers: {
          Authorization: "jwt " + store.getters.token
        }
      }).then(response => {
    ...
    const onHostUpdateFormSubmit = () => {
      // 将数据提交到后台进行保存，但是先进行连接校验，验证没有问题，再保存
      axios.put(`${settings.host}/host/hosts/${hostUpdateForm.form.id}`, hostUpdateForm.form, {
            headers: {
              Authorization: "jwt " + store.getters.token,
            }
          }
    ...
    const deleteManyHost = ()=>{
      axios.delete(`${settings.host}/host/hosts`, {
          params: {
    ...
    const onHostMoveSubmit = ()=>{
      ...
      axios.patch(`${settings.host}/host/hosts`, {
            id_list: hostSelectedRowKeys.value,
