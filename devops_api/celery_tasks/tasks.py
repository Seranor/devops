...
from devops_api.utils.key import PkeyManager
...
def schedule_task(cmd, hosts_ids):
    ...
    for host_obj in hosts_objs:
        ...
        ssh.connect(hostname=host_obj.ip_addr, port=host_obj.port, username=host_obj.username,
                    pkey=RSAKey.from_private_key(StringIO(private_key)) if isinstance(private_key,
                                                                                      str) else private_key, )
        # cli = host_obj.get_ssh(private_key)
        # code, result = ssh.exec_command(cmd)
